...
script() {
    clear
    echo "Petite voix flippante: Bienvenue dans le Chateau d'Hyrule, prêt à souffrir ?"
    sleep 0.5 && echo "Navi: Tu es prêt ?"
    read Answer
    if [[ $Answer = "$ans" ]]; then
	clear
    ...
    bc
    bbf
    slc3 && echo "Ganon Le Fléau: Tu as gagné petit héro, bien joué !
Cela faisai t longtemps qu'un adversaire de ta trempe n'avait pas vu le jour..."
    slc3 && echo "Ganon Le Fléau: Avant que tu t'en aille, sache une chose. Je ne suis pas le dernier Boss d'Hyrule, il y en a un autre, bien plus puissant que moi.
Si un jour tu croises sa route, cours vite et loin, et peut être auras tu une chance de survivre"
    slc3 && echo "Petite voix flippante: Bravo, tu as réussi, on fait la revanche ?"
    read Answer
}
